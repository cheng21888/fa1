...
import pandas as pd
import numpy as np
from tqdm import tqdm  # 修复：显式导入tqdm函数
...
class AdvancedChanAnalyzer:
    ...
    def _analyze_volume(self) -> Dict:
        ...
        try:
            ...
            volume_trend = 'increasing' if recent_data['volume'].iloc[-5:].mean() > recent_data['volume'].iloc[
                                                                                    -10:-5].mean() else 'decreasing'
            ...
            return {
                ...
                'current_volume_ratio': recent_data['vol_ratio'].iloc[
                    -1] if 'vol_ratio' in recent_data.columns else 1.0,
                'volume_surge': recent_data['volume'].iloc[-1] > recent_data['volume'].mean() * 2
            }
        except:
            return {'volume_trend': 'stable', 'price_volume_correlation': 0, 'current_volume_ratio': 1.0,
                    'volume_surge': False}
    ...
    try:
        ...
        if analyze_button:
            with st.spinner('正在获取数据并分析...'):
                ...
                results = []
                # 修复：现在tqdm可以正常调用，因为已经显式导入
                for symbol, df in tqdm(kline_data.items(), desc="分析股票"):
